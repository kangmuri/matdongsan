		...
		<div class="pagetitle">
			...
			<nav>
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active">커뮤니티 신고글 관리</li>
				</ol>
			</nav>
		</div>
		<!-- End Page Title -->
			...
			<div class="row">
				<div class="col-lg-12">
					...
					<div class="card" style="height: 650px;">
						<div class="card-body">
							<h5 class="card-title">커뮤니티 신고 게시글</h5>
									...
									<table class="table datatable datatable-table">
										<thead>
											<tr>
												...
												<th data-sortable="true" class="red" style="width: 18.564593301435405%;">
													<button class="datatable-sorter">신고수</button>
												</th>
											</tr>
										</thead>
...
//		searchForm이 submit이 되면 해당 이벤트를 중지시키고 페이징 정보를 비동기로 요청하는 스크립트
	const $searchForm = $("#searchForm").on('submit',function(e){
		...
		let url = cPath+"/adminCommunity?"+params;
//      console.log("url==>",url);
...
const gen = (vo) =>{
    let date = vo.pstWrtDe.substr(0,10);
   	...
   	str += `<tr>
				<td>\${vo.rnum}</td>
				<td>\${vo.userId}</td>
				<td><a href=\${cPath}/adminCommunity/adminCommunityDetail/\${vo.pstNo}>\${vo.pstSj}</a></td>
				<td>\${date}</td>`;
	if(vo.pstRdcnt == 0){
		str += `<td>0</td>`;	
	}else{
		str += `<td>${vo.pstRdcnt}</td>`;	
	}
	str += `	<td>\${vo.rcnt}</td>
	        </tr>`;
    return str;
}
let url = cPath+`/adminCommunity`;
