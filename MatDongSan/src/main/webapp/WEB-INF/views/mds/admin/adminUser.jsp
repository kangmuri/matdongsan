		...
		<div class="pagetitle">
			...
			<nav>
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item active">회원 계정관리</li>
				</ol>
			</nav>
		</div>
		<!-- End Page Title -->
							...
							<div class="datatable-wrapper datatable-loading no-footer sortable searchable fixed-columns">
								<div class="datatable-top"  style="display: flex; flex-direction: row-reverse;">
									...
									<div id="searchUI"  style="display: flex;">
										<select name="searchType" class="form-select" style="width: 94px; height: 33px; margin: 0px 10px 0px 0px; font-size: 14px;">
								...
								<div class="datatable-container">
									<table class="table datatable datatable-table">
										<thead>
											<tr>
												<th data-sortable="true" style="width: 8.794258373205743%;">
												...
												<th data-sortable="true" style="width: 10.51674641148325%;">
													<button class="datatable-sorter">생년월일</button>
												</th>
												<th data-format="YYYY/DD/MM" data-sortable="true" data-type="date" style="width: 8%;">
													<button class="datatable-sorter">성별</button>
												</th>
												<th data-sortable="true" class="red" style="width: 15.564593301435405%;">
													<button class="datatable-sorter">이메일</button>
												</th>
												<th data-sortable="true" class="red" style="width: 15.564593301435405%;">
													<button class="datatable-sorter">가입일자</button>
												</th>
											</tr>
										</thead>
...
//		searchForm이 submit이 되면 해당 이벤트를 중지시키고 페이징 정보를 비동기로 요청하는 스크립트
	const $searchForm = $("#searchForm").on('submit',function(e){
		...
		let url = cPath+"/adminUser?"+params;
//      console.log("url==>",url);
...
const gen = (vo) =>{
    let date = vo.userCreatOn.substr(0,10);
    let bir = vo.userBir.substr(0,10);
    return `<tr>
				<td>\${vo.rnum}</td>
				<td><a href=\${cPath}/adminUser/\${vo.userNo}>\${vo.userId}</a></td>
				<td>\${vo.userNm}</td>
				<td>\${bir}</td>
				<td>\${vo.userSexdstn}</td>
				<td>\${vo.userEmail}</td>
				<td>\${date}</td>
			</tr>`;
}
let url = cPath+`/adminUser`;
